...
from primpt.data.split import (
    build_external_benchmark,
    create_positive_aware_guide_disjoint_kfold,
)
from primpt.data.split_CHANGEseq import create_changeseq_5fold
...
def main() -> None:
    ...
    if task == "changeseq_canonical_guide_disjoint_kfold":
        create_changeseq_5fold(
            pkl_path=cfg["data"]["input_path"],
            dataset_name=cfg["data"].get("dataset_name", "CHANGEseq"),
            random_seed=cfg["split"].get("random_seed", 100),
            expected_seq_len=cfg["data"].get("expected_seq_len", 23),
            canonical_guide_len=cfg["data"].get("canonical_guide_len", 20),
            expected_canonical_guides=cfg["data"].get("expected_canonical_guides", 110),
            duplicate_policy=cfg["cleaning"].get("duplicate_policy", "drop_conflicts"),
            n_outer_trials=cfg["split"].get("n_outer_trials", 200000),
            n_inner_trials=cfg["split"].get("n_inner_trials", 100000),
            target_val_fraction=cfg["split"].get("target_val_fraction", 0.20),
            guide_count_tolerance=cfg["split"].get("guide_count_tolerance", 2),
            output_root=cfg["output"].get("output_root", "data/processed"),
        )

    elif task == "guide_disjoint_kfold":
        create_positive_aware_guide_disjoint_kfold(
            ...
            val_guide_count_options=tuple(
                cfg["split"].get("val_guide_count_options", [2, 3])
            ),
            target_val_fraction=cfg["split"].get("target_val_fraction", 0.20),
            output_root=cfg["output"].get("output_root", "data/processed"),
        ...
        build_external_benchmark(
            ...
            n_train_val_search_trials=cfg["split"].get(
                "n_train_val_search_trials", 300000
            ),
