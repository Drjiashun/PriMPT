...
class PairPriorBuilder:
    ...
    def __init__(
        ...
        # 优化去冗余后的 1-gram 物理特征维度定义（由 113 维精简至 96 维）。
        ...
        # 优化去冗余后的 2-gram 物理特征维度定义（精简至 53 维）。
        ...
        # 优化去冗余后的 3-gram 物理特征维度定义（精简至 56 维）。
    ...
    def _init_lookup_tables(self) -> None:
        ...
        for g in bases:
            for t in bases:
                ...
                # 生物学逻辑修正：GC 富集结合能强，容忍度高，错配严重度降低。
    ...
    def _build_core_arrays(self, guide_seq: str, target_seq: str) -> Dict[str, np.ndarray]:
        ...
        return self._build_core_arrays_from_indices(guide_idx, target_idx)

    def _build_core_arrays_from_indices(
            self,
            guide_idx: np.ndarray,
            target_idx: np.ndarray,
    ) -> Dict[str, np.ndarray]:
        """Result-equivalent fast path for prevalidated integer base arrays."""
        guide_idx = np.asarray(guide_idx, dtype=np.int64)
        target_idx = np.asarray(target_idx, dtype=np.int64)
        if guide_idx.shape != (self.seq_len,) or target_idx.shape != (self.seq_len,):
            raise ValueError(
                f"Expected integer sequence arrays of shape ({self.seq_len},), "
                f"got {guide_idx.shape} and {target_idx.shape}"
            )
        pair_idx = guide_idx * 4 + target_idx
        return self._build_core_arrays_from_pair_indices(pair_idx)

    def _build_core_arrays_from_pair_indices(self, pair_idx: np.ndarray) -> Dict[str, np.ndarray]:
        """
        Fastest result-equivalent core path. A paired-column index p in [0,15]
        uniquely stores guide=floor(p/4) and target=p mod 4.
        """
        pair_idx = np.asarray(pair_idx, dtype=np.int64)
        if pair_idx.shape != (self.seq_len,):
            raise ValueError(f"Expected pair_idx shape ({self.seq_len},), got {pair_idx.shape}")

        guide_idx = pair_idx // 4
        target_idx = pair_idx % 4
    ...
    def _build_pair_prior_1gram_fast(self, arr: Dict[str, np.ndarray], global_feat: np.ndarray) -> np.ndarray:
        ...
        # 2. Relation (2维) - 去除了 relation_match
        pair_scalars = arr["pair_scalars"]
        out[:, c:c + 2] = pair_scalars[:, 1:3];
        c += 2  # relation_transition, relation_transversion

        # 3. Pair Class (4维)
        out[:, c:c + 4] = pair_scalars[:, 3:7];
        c += 4

        # 4. Mech Mismatch/Wobble + Severity (9维)
        out[:, c:c + 9] = pair_scalars[:, 7:16];
        c += 9

        # 5. Smooth Position (7维) - 移除了数学共线性位置特征
        ...
        # 8. Severity-Position Interactions (2维) - 只保留最核心无共线性的两个交互特征
        out[:, c] = severity * self._rel_pos;
        c += 1
        out[:, c] = severity * self._center_proximity;
        c += 1

        # 9. Local Topology & Neighborhood (15维)
        local_arrays = (
            arr["local_mm_w3"],
            arr["local_mm_w5"],
            arr["local_mm_w7"],
        ...
        # 11. GC Context (4维) - 移除了具有线性代数依存的 pair_gc_mean
        gc_arrays = (
            arr["guide_gc"],
            arr["target_gc"],
    ...
    def _build_tau2_fast(self, arr: Dict[str, np.ndarray], global_feat: np.ndarray) -> np.ndarray:
        ...
        c += 1
        # 去除了 RBF 投影差值（由 relative_position 差值替代）
    ...
    def encode_pair_priors_from_pair_indices(self, pair_idx: np.ndarray) -> Dict[str, np.ndarray]:
        """Compute the original priors from compact paired-column indices."""
        arr = self._build_core_arrays_from_pair_indices(pair_idx)
        global_feat = self._global_topology_features(arr)
        pos_prior = self._build_pair_prior_1gram_fast(arr, global_feat)
        tau2 = self._build_tau2_fast(arr, global_feat)
        tau3 = self._build_tau3_fast(arr, global_feat)
    ...
    def encode_pair_priors_from_indices(
            self,
            guide_idx: np.ndarray,
            target_idx: np.ndarray,
    ) -> Dict[str, np.ndarray]:
        """Compute original priors from prevalidated A=0,C=1,G=2,T=3 arrays."""
        guide_idx = np.asarray(guide_idx, dtype=np.int64)
        target_idx = np.asarray(target_idx, dtype=np.int64)
        pair_idx = guide_idx * 4 + target_idx
        return self.encode_pair_priors_from_pair_indices(pair_idx)
...
# ===================== 3. Tokenizer + Centralized Pair Priors =====================
class PairPriorTokenizer:
    ...
    def encode_from_pair_indices(self, pair_idx: np.ndarray) -> Dict[str, object]:
        """
        Fast result-equivalent encoding from compact paired-column indices.
        No learned state, randomness, or pair-prior cache is involved.
        """
        pair_idx = np.asarray(pair_idx, dtype=np.int64)
        if pair_idx.shape != (self.seq_len_no_cls,):
            raise ValueError(
                f"Expected pair_idx shape ({self.seq_len_no_cls},), got {pair_idx.shape}"
            )

        L = self.seq_len_no_cls
        pair_1gram = np.zeros(L + 1, dtype=np.int64)
        pair_1gram[0] = self.pair_vocab_1gram["[CLS]"]
        pair_1gram[1:] = 3 + pair_idx

        pair_2gram = np.zeros(L + 1, dtype=np.int64)
        pair_2gram[0] = self.pair_vocab_2gram["[CLS]"]
        pair_2gram[1:L] = 3 + pair_idx[:-1] * 16 + pair_idx[1:]

        pair_3gram = np.zeros(L + 1, dtype=np.int64)
        pair_3gram[0] = self.pair_vocab_3gram["[CLS]"]
        pair_3gram[1:L - 1] = 3 + pair_idx[:-2] * 256 + pair_idx[1:-1] * 16 + pair_idx[2:]

        pair_priors = self.pair_prior_builder.encode_pair_priors_from_pair_indices(pair_idx)
        return {
            "pair_1gram": pair_1gram,
            "pair_2gram": pair_2gram,
            "pair_3gram": pair_3gram,
    ...
    def encode_from_indices(
            self,
            guide_idx: np.ndarray,
            target_idx: np.ndarray,
    ) -> Dict[str, object]:
        """Result-equivalent wrapper for prevalidated integer base arrays."""
        guide_idx = np.asarray(guide_idx, dtype=np.int64)
        target_idx = np.asarray(target_idx, dtype=np.int64)
        if guide_idx.shape != (self.seq_len_no_cls,) or target_idx.shape != (self.seq_len_no_cls,):
            raise ValueError(
                f"Expected integer sequence arrays of shape ({self.seq_len_no_cls},), "
                f"got {guide_idx.shape} and {target_idx.shape}"
            )
        pair_idx = guide_idx * 4 + target_idx
        return self.encode_from_pair_indices(pair_idx)



def validate_fast_encoding_equivalence(
        tokenizer: PairPriorTokenizer,
        n_random: int = 256,
        seed: int = 12345,
) -> None:
    """
    Optional audit: fast pair-index path must be bitwise identical to the original
    string path for token IDs and all three prior tensors.
    """
    rng = np.random.default_rng(seed)
    bases = np.asarray(tokenizer.BASES)

    for _ in range(n_random):
        guide_idx = rng.integers(0, 4, size=tokenizer.seq_len_no_cls, dtype=np.int64)
        target_idx = rng.integers(0, 4, size=tokenizer.seq_len_no_cls, dtype=np.int64)
        guide = "".join(bases[guide_idx].tolist())
        target = "".join(bases[target_idx].tolist())

        reference = tokenizer.encode(guide, target)
        fast = tokenizer.encode_from_indices(guide_idx.astype(np.uint8), target_idx.astype(np.uint8))
        pair_idx = (guide_idx * 4 + target_idx).astype(np.uint8)
        fast_pair = tokenizer.encode_from_pair_indices(pair_idx)

        for key in ["pair_1gram", "pair_2gram", "pair_3gram"]:
            ref_arr = np.asarray(reference[key])
            if not np.array_equal(ref_arr, np.asarray(fast[key])):
                raise AssertionError(f"Fast encoding mismatch for {key}")
            if not np.array_equal(ref_arr, np.asarray(fast_pair[key])):
                raise AssertionError(f"Pair-index encoding mismatch for {key}")

        for key in ["pair_prior_1gram", "pair_prior_2gram", "pair_prior_3gram"]:
            if not np.array_equal(reference[key], fast[key]):
                max_abs = float(np.max(np.abs(reference[key] - fast[key])))
                raise AssertionError(f"Fast prior mismatch for {key}; max_abs={max_abs}")
            if not np.array_equal(reference[key], fast_pair[key]):
                max_abs = float(np.max(np.abs(reference[key] - fast_pair[key])))
                raise AssertionError(f"Pair-index prior mismatch for {key}; max_abs={max_abs}")

    print(f"Fast encoding equivalence audit passed for {n_random} random pairs.")
