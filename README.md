# PriMPT: Prior-Informed Multi-Scale Paired-Token Representation Learning for CRISPR/Cas9 Off-Target Cleavage Activity Prediction
PriMPT is a prior-informed multi-scale paired-token representation learning framework for CRISPR/Cas9 off-target cleavage activity prediction. The model represents aligned sgRNA–DNA sequence pairs as multi-scale paired tokens, including 1-gram, 2-gram, and 3-gram guide–target paired tokens. Mechanism-related pair-prior descriptors are injected into token representations to improve the modeling of guide–target pairing status, mismatch chemistry, local mismatch topology, PAM-related context, and sequence-level mismatch burden.  
## Data Sources
Six public CRISPR/Cas9 guide-target cleavage activity datasets were used in this study, including CHANGE-seq, HEK293T, K562, CRISPOR, II4, and II5.
...
```bash
python scripts/prepare_data.py --config configs/split/CHANGEseq.yaml
